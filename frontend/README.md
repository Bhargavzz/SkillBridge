...
Next.js App Router application. Sends analysis requests to the FastAPI backend and renders the live LangGraph execution trace via Server-Sent Events (SSE).
...
├── tailwind.config.ts
├── tsconfig.json
└── next.config.ts            # output: 'standalone' enabled for Docker
```
...
> Next.js is built with `output: 'standalone'` in `next.config.ts` — this produces a self-contained `server.js` that runs without `node_modules`, keeping the Docker image small.
...
- **Standalone output** — `next.config.ts` sets `output: 'standalone'` so the Docker image only bundles what's needed to run the server, not the full `node_modules`.
